...
void	CFTest()
{
	...
	if (kTest_LocToEncoding) {
		...
		for (SStringVec::iterator it = langVec.begin(); it != langVec.end(); ++it) {
			...
			SuperString				formatStr("Convert Locale Encoding: Ò%sÓ (%s, %s)", kCFStringEncodingUTF8);
	...
	if (kTest_Locale) {
		...
		for (SStringMap::iterator it = langVec.begin(); it != langVec.end(); ++it) {
			...
			SuperString			formatStr("Convert Locale to Lang ID: Ò%sÓ", kCFStringEncodingUTF8);
